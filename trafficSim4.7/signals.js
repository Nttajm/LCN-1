...
const SIGNAL_VISIBILITY_PRESETS = [
  { id: 'off', label: 'Off', opacity: 0, iconOpacity: 0.18 },
  { id: 'faint', label: 'Faint', opacity: 0.26, iconOpacity: 0.42 },
  { id: 'medium', label: 'Medium', opacity: 0.62, iconOpacity: 0.72 },
  { id: 'full', label: 'Full', opacity: 1.0, iconOpacity: 1.0 }
];
...
const SIGNAL_VIS_STORAGE_KEY = 'trafficSim4_7_signal_vis';
const SIGNALS_MASTER_STORAGE_KEY = 'trafficSim4_7_signals_enabled';

let signalsEnabled = true;
let signalVisibilityMode = 0; // index into SIGNAL_VISIBILITY_PRESETS — default Off
try {
  const savedVis = localStorage.getItem(SIGNAL_VIS_STORAGE_KEY);
  const visIdx = SIGNAL_VISIBILITY_PRESETS.findIndex(p => p.id === savedVis);
  if (visIdx >= 0) signalVisibilityMode = visIdx;
  const savedMaster = localStorage.getItem(SIGNALS_MASTER_STORAGE_KEY);
  if (savedMaster === '0') signalsEnabled = false;
  if (savedMaster === '1') signalsEnabled = true;
} catch (_) {}
...
function signalsAreDrawn() {
  return getSignalIdleOpacity() > 0.001;
}
...
const LEFT_MODE_CYCLE = ['auto', 'permissive', 'protPerm', 'protected'];
const LEFT_MODE_LABEL = {
  auto: 'Auto',
  permissive: 'Perm',
  protPerm: 'P/P',
  protected: 'Prot'
...
function syncJunctionSignals(nodeKey, nodeX, nodeY, directions, laneNodes, edges) {
  ...
  if (!nd.signal) {
    nd.signal = {
      enabled: true,
      rightOnRed: true,
      protectedLeft: false, // legacy mirror: true if any approach needs protection
      leftModePolicy: 'auto', // auto | permissive | protPerm | protected
      leftModeBySeg: {},
      groupGreen: [],
  ...
  const sig = nd.signal;
  if (sig.leftModePolicy == null) sig.leftModePolicy = 'auto';
  if (!sig.leftModeBySeg || typeof sig.leftModeBySeg !== 'object') sig.leftModeBySeg = {};
  ...
  if (!sig.timing) sig.timing = Object.assign({}, DEFAULT_TIMING);
  // Fill any missing timing keys from defaults
  Object.keys(DEFAULT_TIMING).forEach(k => {
    if (sig.timing[k] == null || !isFinite(sig.timing[k])) sig.timing[k] = DEFAULT_TIMING[k];
  });
  ...
  // Barrier groups: pair opposing approaches (NEMA barrier sides)
  ...
  // Auto left-turn modes + green splits (geometry-based NEMA-style plan)
  autoConfigureSignalPlan(sig, directions, bySeg, groups);
...
/**
 * Resolve left-turn treatment per FHWA-style geometric warrants:
 * - protected: double left, or opposing thru >= 3 lanes
 * - protPerm (protected/permissive): exclusive left lane, or opposing thru >= 2
 * - permissive: shared lane, light opposing volume proxy
 * Junction policy can force all approaches to one mode.
 */
function autoConfigureSignalPlan(sig, directions, bySeg, groups) {
  function enterLaneCount(segId) {
    const arr = bySeg.get(segId);
    return arr ? arr.length : 0;
  }

  function exclusiveLeftLaneCount(segId) {
    return (sig.heads || [])
      .filter(h => h.segId === segId
        && h.movements.length === 1
        && h.movements[0] === 'left')
      .reduce((n, h) => n + (h.laneIdxs ? h.laneIdxs.length : 1), 0);
  }

  function hasAnyLeft(segId) {
    return (sig.heads || []).some(h =>
      h.segId === segId && h.movements.includes('left')
    );
  }

  function opposingThruLanes(segId, groupSegs) {
    const partner = (groupSegs || []).find(s => s !== segId);
    if (partner == null) return 0;
    // Thru-capable enter lanes on the opposing approach
    let n = 0;
    (sig.heads || []).forEach(h => {
      if (h.segId !== partner) return;
      if (h.movements.includes('straight') || h.movements.includes('right')) {
        n += h.laneIdxs ? h.laneIdxs.length : 1;
      }
    });
    if (n === 0) n = enterLaneCount(partner);
    return n;
  }

  function pickModeForSeg(segId, groupSegs) {
    if (!hasAnyLeft(segId)) return 'permissive';
    const leftOnly = exclusiveLeftLaneCount(segId);
    const oppThru = opposingThruLanes(segId, groupSegs);
    // Protected-only warrants (geometry proxies for FHWA guidance)
    if (leftOnly >= 2 || oppThru >= 3) return 'protected';
    // Protected/permissive when exclusive left exists or 2+ opposing thrus
    if (leftOnly >= 1 || oppThru >= 2) return 'protPerm';
    return 'permissive';
  }

  const policy = sig.leftModePolicy || 'auto';
  const leftModeBySeg = {};
  (groups || []).forEach(groupSegs => {
    groupSegs.forEach(segId => {
      if (policy === 'auto') {
        leftModeBySeg[segId] = pickModeForSeg(segId, groupSegs);
      } else {
        leftModeBySeg[segId] = hasAnyLeft(segId) ? policy : 'permissive';
      }
    });
  });
  sig.leftModeBySeg = leftModeBySeg;
  sig.protectedLeft = Object.keys(leftModeBySeg).some(k => {
    const m = leftModeBySeg[k];
    return m === 'protPerm' || m === 'protected';
  });

  // Green splits: weight by enter-lane count (major corridor gets more green)
  const ranks = (groups || []).map(g =>
    g.reduce((s, segId) => s + Math.max(1, enterLaneCount(segId)), 0)
  );
  const rankSum = ranks.reduce((a, b) => a + b, 0) || 1;
  const greenBudget = 20; // total green seconds across barrier sides per cycle
  const nGroups = ranks.length;
  const prevGreen = Array.isArray(sig.groupGreen) ? sig.groupGreen : [];
  // Keep user / map-saved greens when timing was manually set (or restored from save).
  // Only auto-split when still in auto mode, or when barrier count changed.
  if (sig.timingManual && prevGreen.length === nGroups && nGroups > 0) {
    sig.groupGreen = prevGreen.slice();
  } else if (sig.timingManual && sig.timing && isFinite(sig.timing.green) && nGroups > 0) {
    sig.groupGreen = ranks.map(() => sig.timing.green);
  } else {
    sig.groupGreen = ranks.map(r => {
      const share = r / rankSum;
      return Math.round((Math.max(6, Math.min(16, 4 + greenBudget * share)) + Number.EPSILON) * 10) / 10;
    });
  }

  // Clearance timing from crossing width proxy (total enter lanes at junction)
  const totalEnter = ranks.reduce((a, b) => a + b, 0);
  if (!sig.timingManual) {
    sig.timing.yellow = totalEnter >= 8 ? 4.0 : (totalEnter >= 5 ? 3.5 : 3.2);
    sig.timing.allRed = totalEnter >= 8 ? 2.0 : (totalEnter >= 5 ? 1.6 : 1.5);
    sig.timing.protLeft = sig.protectedLeft
      ? (Object.values(leftModeBySeg).some(m => m === 'protected') ? 6.0 : 5.0)
      : DEFAULT_TIMING.protLeft;
    // Keep user-edited base green as fallback for groups without split
    if (sig.timing.green == null) sig.timing.green = DEFAULT_TIMING.green;
  }
}

function phaseAllowFor(phase, segId) {
  if (!phase) return { left: 'red', straight: 'red', right: 'red' };
  if (phase.byApproach && segId != null && phase.byApproach[segId]) {
    return phase.byApproach[segId];
  }
  return phase.allow || { left: 'red', straight: 'red', right: 'red' };
}
...
function rebuildPhaseRing(sig) {
  ...
  const modes = sig.leftModeBySeg || {};

  groups.forEach((groupSegs, gi) => {
    const greenDur = (sig.groupGreen && sig.groupGreen[gi] != null)
      ? sig.groupGreen[gi]
      : (t.green || DEFAULT_TIMING.green);

    const protSegs = groupSegs.filter(sid => {
      const mode = modes[sid] || 'permissive';
      if (mode !== 'protPerm' && mode !== 'protected') return false;
      return (sig.heads || []).some(h => h.segId === sid && h.movements.includes('left'));
    });

    // Leading protected lefts — concurrent for both rings in this barrier
    // (NEMA lead-lead: odd phases fire together before complementary thrus).
    if (protSegs.length) {
      const byProt = {};
      const byProtY = {};
      groupSegs.forEach(sid => {
        if (protSegs.indexOf(sid) >= 0) {
          byProt[sid] = { left: 'green', straight: 'red', right: 'red' };
          byProtY[sid] = { left: 'yellow', straight: 'red', right: 'red' };
        } else {
          byProt[sid] = { left: 'red', straight: 'red', right: 'red' };
          byProtY[sid] = { left: 'red', straight: 'red', right: 'red' };
        }
      });
      phases.push({
        kind: 'protLeft',
        groupIndex: gi,
        duration: t.protLeft || DEFAULT_TIMING.protLeft,
      ...
      phases.push({
        kind: 'protLeftYellow',
        groupIndex: gi,
        duration: t.yellow || DEFAULT_TIMING.yellow,
        allow: { left: 'yellow', straight: 'red', right: 'red' },
        byApproach: byProtY
      });
    }

    // Main concurrent thru/right (dual-ring even phases within the barrier)
    const byGreen = {};
    const byYellow = {};
    groupSegs.forEach(sid => {
      const mode = modes[sid] || 'permissive';
      const hasLeft = (sig.heads || []).some(h =>
        h.segId === sid && h.movements.includes('left')
      );
      let leftState = 'red';
      if (hasLeft) {
        if (mode === 'protected') leftState = 'red'; // already served; no permissive
        else leftState = 'green'; // permissive or P/P permissive portion
      }
      byGreen[sid] = { left: leftState, straight: 'green', right: 'green' };
      byYellow[sid] = {
        left: leftState === 'green' ? 'yellow' : 'red',
        straight: 'yellow',
        right: 'yellow'
      };
    });
    ...
    phases.push({
      ...
      groupIndex: gi,
      duration: t.yellow || DEFAULT_TIMING.yellow,
      allow: { left: 'yellow', straight: 'yellow', right: 'yellow' },
      byApproach: byYellow
    });
    phases.push({
      kind: 'allRed',
      groupIndex: gi,
      duration: t.allRed || DEFAULT_TIMING.allRed,
  ...
  if (phases.length === 0) {
    phases.push({
      kind: 'green',
      groupIndex: 0,
      duration: t.green || DEFAULT_TIMING.green,
      allow: { left: 'green', straight: 'green', right: 'green' }
    });
  }
  sig.phases = phases;
...
function movementDisplay(nodeKey, segId, laneIdx, turn) {
  ...
  const allow = phaseAllowFor(phase, head.segId);
  return allow[move] || 'red';
}
...
function drawAllSignalsCanvas(c) {
  ...
  for (let ei = 0; ei < entries.length; ei++) {
    ...
    if (typeof pointInView === 'function' && !pointInView(nx, ny, vp, 50)) continue;
...
function hitTestSignalAt(worldX, worldY) {
  ...
  for (const [nodeKey, nd] of nodes) {
    ...
    // Heads are only clickable when lights are visible (or this junction is selected).
    if (drawn || nodeKey === selectedSignalNodeKey) {
...
function toggleSignalsMaster() {
  ...
  try {
    localStorage.setItem(SIGNALS_MASTER_STORAGE_KEY, signalsEnabled ? '1' : '0');
  } catch (_) {}
  const btn = document.getElementById('btn-signals');
  if (btn) {
    btn.textContent = signalsEnabled ? 'On' : 'Off';
...
function cycleSignalVisibility() {
  ...
  try {
    ...
    localStorage.setItem(SIGNAL_VIS_STORAGE_KEY, preset.id);
  } catch (_) {}
...
function refreshSignalVisibilityUI() {
  ...
  if (btn) {
    ...
    btn.classList.toggle('active', preset.opacity > 0.001);
  }
  const panelBtn = document.getElementById('sig-vis-panel');
  if (panelBtn) {
    panelBtn.textContent = preset.label;
    panelBtn.classList.toggle('active', preset.opacity > 0.001);
  }
  const viewBtn = document.getElementById('view-btn-lights');
  if (viewBtn) {
    viewBtn.classList.toggle('active', preset.opacity > 0.001);
    viewBtn.title = 'Traffic lights: ' + preset.label + ' (click to cycle)';
  }
  const icon = document.getElementById('view-tl-icon');
  if (icon) {
    icon.style.opacity = String(preset.iconOpacity != null ? preset.iconOpacity : 0.18);
  }
}
...
function updateSignalPanel() {
  ...
  if (plBtn) {
    const policy = sig.leftModePolicy || 'auto';
    plBtn.textContent = LEFT_MODE_LABEL[policy] || 'Auto';
    plBtn.classList.toggle('active', policy !== 'permissive');
    plBtn.title = 'Left-turn mode: Auto (geometry warrants) → Permissive → Protected/Permissive → Protected-only';
  }

  // Show auto-resolved summary under timing when useful
  const modeHint = document.getElementById('sig-leftmode-hint');
  if (modeHint) {
    const modes = sig.leftModeBySeg || {};
    const vals = Object.keys(modes).map(k => modes[k]);
    const uniq = [...new Set(vals)];
    if (!vals.length) modeHint.textContent = '';
    else if (uniq.length === 1) modeHint.textContent = 'Approaches: ' + (LEFT_MODE_LABEL[uniq[0]] || uniq[0]);
    else modeHint.textContent = 'Mixed: ' + uniq.map(m => LEFT_MODE_LABEL[m] || m).join(' / ');
  }
...
function toggleSignalEnabled() {
  ...
  nd.signal.signalManual = true;
  // Lights On owns the intersection — strip stop/yield/ROW so signs don't overlap
  if (nd.signal.enabled && typeof clearApproachControls === 'function') {
    clearApproachControls(nd);
    nd._preferSignals = true;
    nd.controlAutoName = 'Traffic signals';
    nd.controlPresetIdx = null;
    if (typeof refreshJunctionControlSigns === 'function') {
      refreshJunctionControlSigns(selectedSignalNodeKey);
    }
  }
  updateSignals(0);
  updateSignalPanel();
...
function toggleSignalProtLeft() {
  ...
  const nd = nodes.get(selectedSignalNodeKey);
  if (!nd || !nd.signal) return;
  const cur = nd.signal.leftModePolicy || 'auto';
  const idx = LEFT_MODE_CYCLE.indexOf(cur);
  const next = LEFT_MODE_CYCLE[(idx < 0 ? 0 : idx + 1) % LEFT_MODE_CYCLE.length];
  nd.signal.leftModePolicy = next;
  // Re-run plan + phases with current geometry
...
function applySignalTiming() {
  ...
  nd.signal.timingManual = true; // stop auto clearance overwrite until resync geometry wants refresh
  // Apply uniform green to all barrier groups when user edits
  if (nd.signal.groups && nd.signal.groups.length) {
    nd.signal.groupGreen = nd.signal.groups.map(() => nd.signal.timing.green);
  }
