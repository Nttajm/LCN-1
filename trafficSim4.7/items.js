...
const PARK_LANE_GAP = 3.12;
const PARK_FILL = 'rgba(48, 52, 58, 0.22)';
const PARK_STROKE = 'rgba(255, 255, 255, 0.72)';
const PARK_LABEL = 'rgba(245, 245, 245, 0.42)';
const PARK_GHOST_FILL = 'rgba(48, 52, 58, 0.14)';
const PARK_GHOST_STROKE = 'rgba(255, 255, 255, 0.48)';
const PARK_GHOST_LABEL = 'rgba(245, 245, 245, 0.32)';
const PARK_DELETE_FILL = 'rgba(231, 76, 60, 0.28)';
const PARK_DELETE_STROKE = 'rgba(255, 120, 100, 0.95)';
const PARK_ACCENT = '#7f8c9a';
// Aisle-edge T paint: stem runs curb→outer base; half-bar along curb (stacks into a full T)
const PARK_T_BAR_FRAC = 0.22;
const PARK_T_BAR_MIN = 0.85;
...
(function injectParkingStyles() {
  ...
  style.textContent = `
    ...
    .parking-stall-icon span:nth-child(1) {
      left: 0; top: 0; width: 2px; height: 100%;
    }
    .parking-stall-icon span:nth-child(2) {
      left: 0; top: 2px; width: 9px; height: 2px;
    }
    .parking-stall-icon span:nth-child(3) {
      left: 0; bottom: 2px; width: 9px; height: 2px;
    }
    #parking-hud {
      position: fixed;
      top: 18px;
    ...
    #parking-hud .parking-meta { color: var(--ui-muted); }
    #parking-hud kbd {
...
function appendStallMark(ctx, corners, fill, stroke, dash) {
  ...
  const ldx = c1.x - c0.x, ldy = c1.y - c0.y;
  const L = Math.hypot(ldx, ldy) || 1;
  const ddx = c3.x - c0.x, ddy = c3.y - c0.y;
  const D = Math.hypot(ddx, ddy) || 1;
  const ux = ldx / L, uy = ldy / L;
  const nx = ddx / D, ny = ddy / D;
  const stemLen = D;
  const barHalf = Math.max(PARK_T_BAR_MIN, L * PARK_T_BAR_FRAC);
  ...
  // Aisle-edge T marks (road-facing side): crossbar along the curb, stem into
  // the stall. Half the bar sits "above" the stem and half "below" so adjacent
  // stalls stack cleanly into a shared divider T.
  function paintT(cx, cy) {
    ctx.moveTo(cx - ux * barHalf, cy - uy * barHalf);
    ctx.lineTo(cx + ux * barHalf, cy + uy * barHalf);
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + nx * stemLen, cy + ny * stemLen);
  }
...
function drawParkingCanvas(ctx) {
  ...
  const vp = (typeof _drawVp !== 'undefined' && _drawVp)
    || (typeof getWorldViewport === 'function' ? getWorldViewport(32) : null);
  ...
  for (let b = 0; b < parkingBays.length; b++) {
    ...
    const bx0 = Math.min(bay.x1, bay.x2);
    const by0 = Math.min(bay.y1, bay.y2);
    const bx1 = Math.max(bay.x1, bay.x2);
    const by1 = Math.max(bay.y1, bay.y2);
    if (typeof aabbIntersectsView === 'function'
        && !aabbIntersectsView(bx0, by0, bx1, by1, vp)) {
      continue;
    }
    let bayAlpha = 1;
    ...
    if (lod === 0) {
      ctx.globalAlpha = bayAlpha * 0.55;
      ctx.fillStyle = PARK_FILL;
      ctx.strokeStyle = PARK_STROKE;
      ctx.lineWidth = 0.45;
      ctx.beginPath();
      ctx.moveTo(bay.x1, bay.y1);
      const c1 = parkingBayCorners(bay, 0);
      const cN = parkingBayCorners(bay, bay.count - 1);
      if (c1 && cN) {
...
function cutParkingByRoad(ax, ay, bx, by, halfW) {
  ...
  parkingBays = next;
  redrawAllParking();
  if (typeof syncCarsAfterParkingGeometryChange === 'function') {
    syncCarsAfterParkingGeometryChange();
  }
...
function parkingClearRange(seg) {
  ...
  if (typeof nodes !== 'undefined' && typeof getNodeKey === 'function') {
    ...
    const startGraph = typeof nodeHasLaneGraph === 'function' ? nodeHasLaneGraph(sn) : (sn && sn.count > 1);
    const endGraph = typeof nodeHasLaneGraph === 'function' ? nodeHasLaneGraph(en) : (en && en.count > 1);
    if (startGraph) {
...
function removeParkingForSegment(segId) {
  ...
  if (typeof invalidateParkingBayIndex === 'function') invalidateParkingBayIndex();
  if (changed && typeof syncCarsAfterParkingGeometryChange === 'function') {
    syncCarsAfterParkingGeometryChange();
  }
  return changed;
}
