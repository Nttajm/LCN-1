...
const ALLIE_CONFIG = {
  ...
  // Exit-room check: don't enter if the receiving lane is backed up
  // (don't-block-the-box), independent of the driver-head cone.
  ...
  IX_EXIT_ROOM: 7.5,          // clear distance required past the turn exit
  IX_EXIT_SCAN: 20,           // how far past turn end to look for the queue tail
  IX_EXIT_SLOW_SPEED: 3.5,    // at/below this = standing queue (not free-flow)
...
const PARKING_CONFIG = {
  ...
  STAGE_TIMEOUT: 6.5,         // stuck staging → abort & keep looking (was 12)
  // Give up a stall early when the stage pose is clearly wrong instead of
  // sitting through the full timeout.
  STAGE_ABORT_LAT: 4.2,       // lateral miss from stage (world units)
  STAGE_MISALIGN_TIME: 1.6,   // seconds of bad geometry before cancel
  STAGE_NO_PROGRESS: 3.2,     // seconds without closing on stage → cancel
...
for (let i = 0; i < 20; i++) _ixPathSamples.push({ s: 0, x: 0, y: 0 });
...
let spawnerPlaceMode = false;      // random-dest spawner: one click
let abSpawnerPlaceMode = false;    // A→B spawner: click origin, then destination
let pendingAbOrigin = null;        // first pick while placing an A→B spawner
...
function updateDrivePointerHover(worldPt) {
  ...
  // Spawner placement owns the click — don't steal it for follow/select.
  if (spawnerPlaceMode || abSpawnerPlaceMode) {
    if (hoveredCar) clearHoveredCar();
    return;
  }
...
function updateDriveHudText() {
  ...
  if (abSpawnerPlaceMode) {
    meta.innerHTML = pendingAbOrigin
      ? 'A→B spawner · click destination · <kbd>Esc</kbd> re-pick origin'
      : 'A→B spawner · click origin (pickup) · <kbd>Esc</kbd> cancel';
    return;
  }
  if (spawnerPlaceMode) {
...
function handleDriveMouseMove(worldPt) {
  ...
  // A→B spawner placement: preview route from pending origin → hover
  if (abSpawnerPlaceMode) {
    if (pendingAbOrigin && pick) drawPreviewRoute(allieFindPath(pendingAbOrigin, pick));
    else clearPreviewRoute();
    return;
  }
...
function handleDriveClick(event) {
  ...
  if (abSpawnerPlaceMode) {
    handleAbSpawnerPlaceClick(pick);
    return;
  }
...
function drawDriveOverlaysCanvas(c) {
  ...
  const exitColor = (typeof COLOR_EXIT !== 'undefined' && COLOR_EXIT) ? COLOR_EXIT : '#ef5555';
  if (spawnPin) {
    canvasFillCircle(c, spawnPin.x, spawnPin.y, 2.2, enterColor, '#fff', 0.6);
  }
  // Pending A→B origin while placing
  if (pendingAbOrigin) {
    canvasFillCircle(c, pendingAbOrigin.x, pendingAbOrigin.y, 2.4, '#ffb347', '#fff', 0.7);
  }
  ...
  for (let i = 0; i < spawners.length; i++) {
    ...
    const isAb = sp.kind === 'ab';
    // A→B: faint route ribbon so the corridor is obvious
    if (isAb && sp.routeCache && sp.routeCache[0]) {
      const ribbon = sampleSpawnerRouteRibbon(sp.routeCache[0]);
      if (ribbon && ribbon.length >= 2) {
        c.save();
        c.globalAlpha = active ? 0.45 : 0.22;
        c.strokeStyle = '#6fd4ff';
        c.lineWidth = 1.1;
        c.lineCap = 'round';
        c.lineJoin = 'round';
        c.setLineDash([2.2, 1.6]);
        c.beginPath();
        c.moveTo(ribbon[0].x, ribbon[0].y);
        for (let p = 1; p < ribbon.length; p++) c.lineTo(ribbon[p].x, ribbon[p].y);
        c.stroke();
        c.setLineDash([]);
        c.restore();
      }
    ...
    c.fillStyle = isAb ? 'rgba(111, 212, 255, 0.16)' : 'rgba(255, 160, 60, 0.18)';
    c.fill();
    c.strokeStyle = active ? (isAb ? '#6fd4ff' : '#ffb347') : '#888';
    c.lineWidth = 0.65;
    ...
    c.fillStyle = active ? (isAb ? '#6fd4ff' : '#ffb347') : '#666';
    c.fill();
    c.restore();
    if (isAb) {
      const dm = sp.destMarker || { x: sp.destX, y: sp.destY };
      if (dm && Number.isFinite(dm.x) && Number.isFinite(dm.y)) {
        c.save();
        c.beginPath();
        c.arc(dm.x, dm.y, 3.0, 0, Math.PI * 2);
        c.fillStyle = 'rgba(239, 85, 85, 0.14)';
        c.fill();
        c.strokeStyle = active ? exitColor : '#888';
        c.lineWidth = 0.65;
        if (!active) c.setLineDash([1.2, 1]);
        c.stroke();
        c.setLineDash([]);
        c.beginPath();
        c.arc(dm.x, dm.y, 1.35, 0, Math.PI * 2);
        c.fillStyle = active ? exitColor : '#666';
        c.fill();
        c.restore();
      }
    }
...
/** Coarse polyline along a cached ALLIE route for A→B overlay ribbons. */
function sampleSpawnerRouteRibbon(route, maxPts) {
  if (!route || !route.length) return null;
  const out = [];
  const budget = maxPts || 48;
  const perLeg = Math.max(2, Math.ceil(budget / route.length));
  for (let i = 0; i < route.length; i++) {
    const leg = route[i];
    if (!leg || !leg.atom || typeof leg.atom.sampleAtT !== 'function') continue;
    const t0 = leg.tStart != null ? leg.tStart : 0;
    const t1 = leg.tEnd != null ? leg.tEnd : 1;
    for (let s = 0; s <= perLeg; s++) {
      if (s === 0 && out.length) continue; // skip duplicate joint
      const frac = t0 + (t1 - t0) * (s / perLeg);
      const p = leg.atom.sampleAtT(frac);
  ...
  const status = car.state === 'despawning'
    ...
    : (car.state === 'parked' ? 'Parked'
      : (car.state === 'parking' ? ('Parking · ' + (car.parkPhase || 'reverse'))
        : ((car._parkRefitStatusT > 0 && car._parkRefitStatus) ? car._parkRefitStatus
...
function collectCarWhyFacts(car) {
  ...
  if (car._ixBlocker) {
    const why = car._ixBlockReason === 'exit' ? 'exit backed up' : 'path';
    facts.push({ k: 'IX blocker', v: '#' + car._ixBlocker.id + ' · ' + why, alert: true });
  }
  if (car._hardSafetyHit) {
    const win = hardSafetyLoser(car, car._hardSafetyHit) === car._hardSafetyHit;
    facts.push({
...
function describeCarAction(car) {
  ...
  else if ((car._parkRefitStatusT || 0) > 0)
    tags.push({ text: "Won't fit — looking for another", color: '#FFB347' });
...
function headAwarenessConstraintFor(car) {
  ...
  if (head.criticalThreat) {
    ...
    if (sameLane || pathLead || parkingInLane) {
      ...
      // Stop-sign final settle: don't alternate Sensor stop / pull-up with the
      // signed approach (esp. with staggered head-scan cache frames).
      if (pullUp) {
        const info = findUpcomingSignalTurn(car);
        const stopDist = info
          ? Math.max(0, info.dist - ALLIE_CONFIG.STOP_LINE_GAP)
          : (isFinite(head.criticalGap) ? head.criticalGap : 0);
        // Only force sensor-zero in the true arrive zone (not mid-block)
        if (stopSignSettlingAtLine(car, stopDist)) {
          return {
            desired: 0,
            decelRate: ALLIE_CONFIG.DECEL_SHARP,
            status: 'Sensor stop'
...
// Extends past the turn exit so the exit-room (don't-block-the-box) scan can see
// a backed-up receiving lane.
function sampleIntersectionPath(car, info) {
  const turnLeg = info.turnLeg;
  const s0 = Math.max(car.traveledLength, turnLeg.cumStart);
  const exitScan = ALLIE_CONFIG.IX_EXIT_SCAN != null ? ALLIE_CONFIG.IX_EXIT_SCAN : 20;
  const pad = Math.max(ALLIE_CONFIG.IX_BOX_PAD || 0, exitScan);
  const s1 = Math.min(car.totalLength, turnLeg.cumEnd + pad);
  let n = 0;
  const step = Math.max(2.2, (s1 - s0) / 14);
...
// only earn caution (slow), never a hard hold. Exit-room (don't-block-the-box):
// hold at the limit line when the receiving lane is backed up, even on turns
// where the queue sits outside the driver-head cone.
function intersectionClearanceConstraintFor(car) {
  ...
  const exitRoom = ALLIE_CONFIG.IX_EXIT_ROOM != null ? ALLIE_CONFIG.IX_EXIT_ROOM : 7.5;
  const exitSlow = ALLIE_CONFIG.IX_EXIT_SLOW_SPEED != null ? ALLIE_CONFIG.IX_EXIT_SLOW_SPEED : 3.5;
  const turnEnd = info.turnLeg.cumEnd;
  const egoX = car._cx != null ? car._cx : car.x;
  const egoY = car._cy != null ? car._cy : car.y;
  const cosH = car._cosH != null ? car._cosH : Math.cos(car.heading);
  const sinH = car._sinH != null ? car._sinH : Math.sin(car.heading);
  ...
  const nearby = collectNearbyCars(egoX, egoY, ALLIE_CONFIG.IX_CLEAR_LOOKAHEAD + 60);
  const frontPeers = (conflicts && info && info.nodeKey)
    ? frontConflictPeers(car, nearby, info.nodeKey, conflicts, myOrigin)
    : null;
  for (let i = 0; i < nearby.length; i++) {
    const other = nearby[i];
    if (other === car || other.isProbe || other.state === 'despawning') continue;
    // Curb parkers on the receiving road must not false-trigger exit holds
    ...
    // Don't-block-the-box: slow car on my receiving lane past the exit with
    // less than IX_EXIT_ROOM of free road. No head-cone / yield-FOV gate —
    // turns put the exit queue well off the approach heading.
    if (bestLatSq <= pathHalfSq && bestS >= turnEnd - 1
        && other.speed <= exitSlow) {
      const room = bestS - turnEnd;
      if (room < exitRoom && room < exitBlockerRoom) {
        exitBlockerRoom = room;
        exitBlocker = other;
      }
    }

    // Path / conflict / caution holds stay inside forward yield FOV (never behind)
    if (!isCarInYieldForwardView(car, other)) continue;
  ...
  // Exit-room hold wins over / merges with path blockers
  ...
  car._ixBlockReason = blockReason;

  if (blocker) {
    if (car._ixHoldSince == null) car._ixHoldSince = simTime;
    const waited = simTime - car._ixHoldSince;
    const exitHold = blockReason === 'exit';
    const stillBlocking = exitHold
      || otherIsInIntersection(blocker, info.nodeKey)
      || blocker.speed > 0.8;
    // Never timeout-release into a full exit — hold until IX_EXIT_ROOM opens
    if (!exitHold && waited >= ALLIE_CONFIG.IX_HOLD_TIMEOUT && !stillBlocking) {
      car._ixHoldSince = null;
      car._ixBlocker = null;
      car._ixBlockReason = null;
      return null;
    }
    if (!exitHold && waited >= ALLIE_CONFIG.IX_HOLD_TIMEOUT && blocker.speed < 0.3) {
...
/** Arrive-zone distance for stop-sign dwell / final settle (slightly past brake pad). */
function stopSignArriveSlop() {
  const pad = ALLIE_CONFIG.STOP_BRAKE_PAD != null ? ALLIE_CONFIG.STOP_BRAKE_PAD : 0;
  return Math.max(1.2, pad + 0.55);
}

/**
 * True when the car is inside the painted-line arrive zone and should finish
 * at 0 for dwell. Do NOT treat "nearly stopped mid-block" as settled — that
 * froze cars short of the intersection after a traffic stop.
 */
function stopSignSettlingAtLine(car, stopDist) {
  if (stopDist == null || !isFinite(stopDist)) return false;
  return stopDist <= stopSignArriveSlop();
}

function clearStopSignApproachLatch(st) {
  if (!st) return;
  st.brakingToStop = false;
  st.lastApproachDesired = null;
  st.pullingToLine = false;
}
...
function trafficConstraintFor(car) {
  ...
  if (obs.other && stopSignLeadPastLimitLine(car, obs.other)) {
    ...
    const stopDist = info
      ? Math.max(0, info.dist - ALLIE_CONFIG.STOP_LINE_GAP)
      : gap;
    // Final settle: only zero out once inside the true arrive zone so a
    // mid-block traffic stop cannot pin the car forever before the line.
    if (stopSignSettlingAtLine(car, stopDist)) {
      car._trafficStatus = 'Stopped for traffic';
      return {
        desired: 0,
        decelRate: ALLIE_CONFIG.DECEL_SHARP,
        status: car._trafficStatus
...
function signedJunctionConstraintFor(car) {
  ...
  if (myControl === 'stop') {
    const st = car.stopSignState;
    const arriveSlop = stopSignArriveSlop();

    if (st.phase === 'approach' || st.phase === 'dwell') {
      const nearLine = stopDist <= arriveSlop;
      const settleNear = stopSignSettlingAtLine(car, stopDist);
      const stoppedEnough = car.speed <= 0.55;

      // Finished the stop → dwell (must be inside the arrive zone)
      if (settleNear && stoppedEnough) {
        if (st.phase !== 'dwell') {
          st.phase = 'dwell';
        ...
        if (nearLine) {
          st.brakingToStop = true;
          st.lastApproachDesired = 0;
          return {
            desired: 0,
            decelRate: ALLIE_CONFIG.DECEL_SHARP,
            status: 'Stop sign'
        ...
        if (stopDist > bite && !st.brakingToStop) {
          clearStopSignApproachLatch(st);
          return {
        ...
        // Inside bite (or already latched). Rules that kill gas↔brake hunt:
        //  1) While rolling: desired never exceeds current speed (coast/brake only).
        //  2) Pull-up from a true dead stop only (speed ≈ 0), up to creep — not
        //     every time speed dips below 0.55 during braking (that re-opened hunt).
        st.brakingToStop = true;
        const rate = ALLIE_CONFIG.DECEL_SHARP;
        const pad = ALLIE_CONFIG.STOP_BRAKE_PAD != null ? ALLIE_CONFIG.STOP_BRAKE_PAD : 0;
        const dAvail = Math.max(0, stopDist - pad);
        const curve = Math.sqrt(Math.max(0, 2 * rate * dAvail));
        const creepCap = Math.min(
          curve,
          ALLIE_CONFIG.JUNCTION_CREEP_SPEED != null ? ALLIE_CONFIG.JUNCTION_CREEP_SPEED : 4.2
        );

        let desired;
        let decel = rate;
        if (car.speed <= 0.08 && stopDist > arriveSlop) {
          // Dead-stopped short of the line (queue cleared) — one pull-up
          st.pullingToLine = true;
          desired = creepCap;
          decel = ALLIE_CONFIG.DECEL_NORMAL;
        } else if (st.pullingToLine && car.speed < creepCap - 0.25 && stopDist > arriveSlop) {
          // Still finishing that pull-up accel; don't jump back to coast-cap yet
          desired = creepCap;
          decel = ALLIE_CONFIG.DECEL_NORMAL;
          if (car.speed >= creepCap - 0.35) st.pullingToLine = false;
        } else {
          // Rolling in the bite: coast or brake only — never gas
          st.pullingToLine = false;
          desired = Math.min(curve, Math.max(0, car.speed));
        }
        st.lastApproachDesired = desired;
        return {
          desired,
          decelRate: decel,
          status: 'Stop sign'
...
function computeDesiredSpeed(car) {
  ...
  // Keep "won't fit" status visible while hunting another stall
  if ((car._parkRefitStatusT || 0) > 0 && car._parkRefitStatus) {
    car._signalStatus = car._parkRefitStatus;
  }
  if (trace) {
...
function clearParkingIntent(car) {
  ...
  car._parkRefitStatus = null;
  car._parkRefitStatusT = 0;
}
...
function resumeParkingRoam(car) {
  ...
  clearParkingIntent(car);
  car.state = 'driving';
  car._stagingBestDist = null;
  car._stagingMisalignT = 0;
...
/**
 * Cancel a bad / unreachable staging attempt, blacklist that stall for this
 * car, and resume curb hunting. Prefer this over waiting out STAGE_TIMEOUT.
 */
function abortBadParkingStage(car, reason) {
  if (!car) return false;
  const plan = car._parkPlan;
  if (plan && plan.bay != null && plan.stallIndex != null) {
    if (!car._parkRoamRejected) car._parkRoamRejected = new Set();
    car._parkRoamRejected.add(plan.bay.id + ':' + plan.stallIndex);
  }
  const why = reason || 'bad stage';
  resumeParkingRoam(car);
  // HUD / tag: car realized this stall won't work and is hunting again
  car._parkRefitStatus = "Won't fit — looking for another";
  car._parkRefitStatusT = 4.5;
  car._signalStatus = car._parkRefitStatus;
  car._parkDebug = { phase: 'roaming', spot: why };
  return true;
}

/**
 * Is the stage pose clearly unreachable / misaligned from the car's current
 * pose? Returns a short reason string, or null if still plausible.
 */
function stagingPoseBadReason(car, sp, dist) {
  if (!car || !sp) return 'no stage';
  ...
  const abortLat = PARKING_CONFIG.STAGE_ABORT_LAT != null
    ? PARKING_CONFIG.STAGE_ABORT_LAT : 4.2;
  const posTol = PARKING_CONFIG.STAGE_POS_TOL;

  // Stage clearly behind the bumper
  if (fwd < -(ALLIE_CONFIG.CAR_LENGTH * 0.35) && dist > posTol) return 'stage behind';
  // Stage far off to the side (pure-pursuit won't line up on-lane)
  if (lat > abortLat && fwd < Math.max(posTol, dist * 0.5)) return 'stage beside';
  // Close enough to commit but heading is way off the planned stage heading
  if (car._parkStageHeading != null && dist <= posTol * 2.8) {
    const headErr = Math.abs(wrapAngle(car._parkStageHeading - car.heading));
    const headLim = (PARKING_CONFIG.STAGE_HEAD_TOL || 0.3) * 2.4;
    if (headErr > headLim) return 'bad heading';
  }
  return null;
}
...
function findParkingCandidate(car) {
  ...
  for (let b = 0; b < bayList.length; b++) {
    ...
    for (let i = 0; i < bay.count; i++) {
      if (rejected && rejected.has(bay.id + ':' + i)) continue;
...
function beginParkingStaging(car, candidate) {
  ...
  if (!plan || !plan.stagePoint) {
    releaseStallReservation(car);
    car._parkPlan = null;
    return false;
  }

  // Reject plans whose stage point is already behind / far beside — car will
  // only get stuck trying to reach it.
  const sp = plan.stagePoint;
  const cosH = Math.cos(car.heading), sinH = Math.sin(car.heading);
  const dx = sp.x - car.x, dy = sp.y - car.y;
  const fwd = dx * cosH + dy * sinH;
  const lat = Math.abs(-dx * sinH + dy * cosH);
  const abortLat = PARKING_CONFIG.STAGE_ABORT_LAT != null
    ? PARKING_CONFIG.STAGE_ABORT_LAT : 4.2;
  if (fwd < ALLIE_CONFIG.CAR_LENGTH * 0.45 || lat > abortLat * 1.35) {
    if (!car._parkRoamRejected) car._parkRoamRejected = new Set();
    car._parkRoamRejected.add(candidate.bay.id + ':' + candidate.stallIndex);
    releaseStallReservation(car);
    car._parkPlan = null;
    return false;
  }
  ...
  car._stagingT = 0;
  car._parkingT = 0;
  car._stagingBestDist = Math.hypot(dx, dy);
  car._stagingMisalignT = 0;
  car._stagingNoProgressT = 0;
  car._parkRefitStatus = null;
  car._parkRefitStatusT = 0;
...
function updateParkingSearch(car, dt) {
  ...
  if (car.parkPhase === 'staging') {
    ...
    if (car._stagingT > PARKING_CONFIG.STAGE_TIMEOUT) {
      abortBadParkingStage(car, 'stage timeout');
      return;
    }
    ...
    if (!sp) {
      abortBadParkingStage(car, 'no stage');
      return;
    }
    ...
    if (car._parkYieldOther && isActiveParker(car._parkYieldOther)) {
      ...
      car._stagingNoProgressT = (car._stagingNoProgressT || 0) + dt;
      const noProg = PARKING_CONFIG.STAGE_NO_PROGRESS != null
        ? PARKING_CONFIG.STAGE_NO_PROGRESS : 3.2;
      if (car._stagingNoProgressT >= noProg) {
        abortBadParkingStage(car, 'no progress');
        return;
      }
    }

    // Early cancel when stage pose is clearly wrong (behind / beside / heading)
    const bad = stagingPoseBadReason(car, sp, dist);
    if (bad) {
      car._stagingMisalignT = (car._stagingMisalignT || 0) + dt;
      const misT = PARKING_CONFIG.STAGE_MISALIGN_TIME != null
        ? PARKING_CONFIG.STAGE_MISALIGN_TIME : 1.6;
      // Instant abort for behind/beside; heading needs a short confirm
      const instant = bad === 'stage behind' || bad === 'stage beside' || bad === 'no stage';
      if (instant || car._stagingMisalignT >= misT) {
        abortBadParkingStage(car, bad);
        return;
      }
    } else {
      car._stagingMisalignT = 0;
    ...
    if (dist <= PARKING_CONFIG.STAGE_POS_TOL && car.speed <= PARKING_CONFIG.STAGE_SPEED_TOL) {
      ...
      if (car._parkStageHeading != null) {
        const headErr = Math.abs(wrapAngle(car._parkStageHeading - car.heading));
        const headLim = (PARKING_CONFIG.STAGE_HEAD_TOL || 0.3) * 1.5;
        if (headErr > headLim) {
          abortBadParkingStage(car, 'bad align');
          return;
        }
      }
...
function updateCar(car, dt) {
  ...
  if ((car._parkRefitStatusT || 0) > 0) {
    car._parkRefitStatusT = Math.max(0, car._parkRefitStatusT - dt);
    if (car._parkRefitStatusT <= 0) {
      car._parkRefitStatus = null;
    } else if (car._parkRefitStatus) {
      car._signalStatus = car._parkRefitStatus;
    }
  }
  car._debugDesired = desired;
...
function buildSpawnerRouteCache(spawner) {
  if (spawner.kind === 'ab') return buildAbSpawnerRouteCache(spawner);
...
/** Fixed A→B route — rematch world snaps onto the live ALLIE graph after rebuilds. */
function buildAbSpawnerRouteCache(spawner) {
  const radius = Math.max(SPAWNER_LANE_RADIUS, spawner.laneRadius || 0);
  const origin = findNearestAtomPoint(spawner.x, spawner.y, radius);
  const dest = findNearestAtomPoint(spawner.destX, spawner.destY, radius);
  if (!origin || !dest) {
    spawner.originPick = null;
    spawner.destPick = null;
    spawner.destCount = 0;
    return [];
  }
  if (origin.atom === dest.atom && Math.hypot(origin.x - dest.x, origin.y - dest.y) < 1.5) {
    spawner.originPick = origin;
    spawner.destPick = dest;
    spawner.destCount = 0;
    return [];
  }
  const route = allieFindPath(origin, dest);
  ...
  spawner.originPick = origin;
  spawner.destPick = dest;
  // Keep markers snapped to live lane centers
  spawner.x = origin.x;
  spawner.y = origin.y;
  spawner.destX = dest.x;
  spawner.destY = dest.y;
  spawner.marker = { x: origin.x, y: origin.y };
  spawner.destMarker = { x: dest.x, y: dest.y };
  spawner.destCount = 1;
  return [route];
}
...
function drawSpawnerMarker(spawner) {
  ...
  if (spawner.kind === 'ab' && Number.isFinite(spawner.destX) && Number.isFinite(spawner.destY)) {
    spawner.destMarker = { x: spawner.destX, y: spawner.destY };
  }
}
...
function formatSpawnerRowTitle(sp) {
  ...
  const kindLabel = sp.kind === 'ab' ? 'A→B' : 'rand';
  return '#' + sp.id + ' · ' + kindLabel + ' · every ' + sp.intervalSec + 's · ' + routes + ' routes · ' + status;
}
...
function toggleSpawnerPlaceMode() {
  if (!driveMode) {
    showDriveToast('Enter Drive mode first');
    return;
  }
  setSpawnerPlaceMode(!spawnerPlaceMode);
}

function clearPendingAbOrigin() {
  pendingAbOrigin = null;
  clearPreviewRoute();
  updateDriveHudText();
}

function setAbSpawnerPlaceMode(on) {
  abSpawnerPlaceMode = !!on;
  if (abSpawnerPlaceMode) {
    setSpawnerPlaceMode(false);
    clearPendingSpawn();
    clearPendingAbOrigin();
  } else {
    clearPendingAbOrigin();
  }
  const btn = document.getElementById('btn-ab-spawner-place');
  if (btn) {
    btn.textContent = abSpawnerPlaceMode ? 'Cancel A→B' : 'Place A→B spawner';
    btn.classList.toggle('active', abSpawnerPlaceMode);
  }
  updateDriveHudText();
}

function toggleAbSpawnerPlaceMode() {
  if (!driveMode) {
    showDriveToast('Enter Drive mode first');
    return;
  }
  setAbSpawnerPlaceMode(!abSpawnerPlaceMode);
}

function handleAbSpawnerPlaceClick(pick) {
  if (!pendingAbOrigin) {
    pendingAbOrigin = pick;
    updateDriveHudText();
    showDriveToast('Origin set — click destination');
    return;
  }
  if (pick.atom === pendingAbOrigin.atom && Math.hypot(pick.x - pendingAbOrigin.x, pick.y - pendingAbOrigin.y) < 1.5) {
    showDriveToast('Pick a destination further down the road');
    return;
  }
  const route = allieFindPath(pendingAbOrigin, pick);
  if (!route || !route.length) {
    showDriveToast('No route found — try a different destination');
    return;
  }
  placeAbSpawner(pendingAbOrigin, pick, route);
  clearPendingAbOrigin();
  setAbSpawnerPlaceMode(false);
}
...
/** Drive-mode-style fixed corridor: keep spawning cars from A to B. */
function placeAbSpawner(originPick, destPick, precomputedRoute) {
  const settings = readSpawnerSettingsFromUI();
  const route = precomputedRoute || allieFindPath(originPick, destPick);
  if (!route || !route.length) {
    showDriveToast('No route found — try a different destination');
    return;
  }
  const spawner = {
    ...
    routeCache: [route],
    destCount: 1,
    marker: { x: originPick.x, y: originPick.y },
    destMarker: { x: destPick.x, y: destPick.y }
  };
  // Rebuild once so rematch + ribbon stay consistent with graph ids
  refreshSpawnerDestCache(spawner);
  if (!spawner.routeCache.length) {
    showDriveToast('No route found — try a different destination');
    return;
  ...
  drawSpawnerMarker(spawner);
  updateSpawnerListUI();
  showDriveToast('A→B spawner #' + spawner.id + ' · cars will keep running this route');
}
...
function importMapSpawners(data) {
  ...
  data.forEach(item => {
    const kind = item.kind === 'ab' ? 'ab' : 'random';
    if (kind === 'ab' && !(Number.isFinite(item.destX) && Number.isFinite(item.destY))) return;
    const spawner = {
      id: item.id || spawnerIdCounter++,
      kind,
      x: item.x,
      y: item.y,
      destX: kind === 'ab' ? item.destX : undefined,
      destY: kind === 'ab' ? item.destY : undefined,
      ...
      destMarker: kind === 'ab' ? { x: item.destX, y: item.destY } : null
    };
