...
const ALLIE_CONFIG = {
  ...
  STOP_SIGN_DWELL: 0.85,       // full stop hold before leaving a stop sign
...
function signedJunctionConstraintFor(car) {
  ...
  function conflictingThreat() {
    ...
    const myArrival = (car.stopSignState && car.stopSignState.arrivalT != null)
      ? car.stopSignState.arrivalT : simTime;
    ...
    for (let i = 0; i < nearby.length; i++) {
      ...
      if (oLeg && oLeg.atom.kind === 'turn' && oLeg.atom.nodeKey === info.nodeKey
        ...
        if (other.stopSignState && other.stopSignState.nodeKey === info.nodeKey) {
          oArrival = other.stopSignState.arrivalT;
        }
      } else {
        const oInfo = findUpcomingSignalTurn(other);
        ...
        conflictsWith = true;
        if (other.stopSignState && other.stopSignState.turnLegIndex === oInfo.turnLegIndex) {
          oArrival = other.stopSignState.arrivalT;
        } else {
      ...
      // Same rank: first arrival (all-way stop / mutual yield)
      // Stops that haven't finished dwell yet don't claim priority over peers
      if (myControl === 'stop' && oControl === 'stop') {
        const oReady = other.stopSignState
          && (other.stopSignState.phase === 'wait' || other.stopSignState.phase === 'cleared'
            || (other.stopSignState.phase === 'dwell' && other.speed < 0.35));
        const iReady = car.stopSignState
          && (car.stopSignState.phase === 'wait' || car.stopSignState.phase === 'dwell');
        if (iReady && oReady) {
          if (oArrival < myArrival - 0.001
              || (Math.abs(oArrival - myArrival) < 0.001 && other.id < car.id)) {
            if (!yieldTo || other.id < yieldTo.id) yieldTo = other;
          }
        } else if (oReady && !iReady) {
          if (!yieldTo || other.id < yieldTo.id) yieldTo = other;
        }
        continue;
      }
  ...
  if (myControl === 'row') {
    const threat = conflictingThreat();
  ...
  // ---- STOP: full stop + dwell, then gap check ----
  if (myControl === 'stop') {
    ...
    if (st.phase === 'approach' || st.phase === 'dwell') {
      ...
      // At the line and nearly stopped
      if (st.phase !== 'dwell') {
        st.phase = 'dwell';
    ...
    car._yieldOther = threat || null;
    if (threat) {
      return { desired: 0, decelRate: ALLIE_CONFIG.SIGNAL_DECEL, status: 'Yielding' };
    }
    // Anti-deadlock
    if (simTime - st.arrivalT >= ALLIE_CONFIG.JUNCTION_YIELD_TIMEOUT + ALLIE_CONFIG.STOP_SIGN_DWELL
        && car.speed < 1.0) {
      car._yieldOther = null;
      return null;
    }
  ...
  // ---- YIELD: no mandatory stop; slow / hold for higher priority or conflicts ----
  {
    const threat = conflictingThreat();
    car._yieldOther = threat || null;
    if (!threat) {
      // Approach cautiously near the line even when clear
      if (stopDist < 10 && car.speed > ALLIE_CONFIG.CRUISE_SPEED * 0.65) {
        return {
          desired: ALLIE_CONFIG.CRUISE_SPEED * 0.55,
          decelRate: ALLIE_CONFIG.DECEL_NORMAL,
          status: 'Yield sign'
    ...
    if (!c) {
      return {
        desired: ALLIE_CONFIG.CRUISE_SPEED * clampNum(stopDist / look, 0.15, 0.5),
        decelRate: ALLIE_CONFIG.DECEL_NORMAL,
        status: 'Yielding'
      };
    }
...
function parkingYieldConstraintFor(car) {
  ...
  for (let i = 0; i < nearby.length; i++) {
    ...
    // Anchors: body (while still near the lane) + fixed stage point.
    // Stage point stays in the travel lane for the whole reverse, so traffic
    // keeps holding even after the parker swings into the stall.
    const anchors = [];
    const sp = other._parkStagePoint || (other._parkPlan && other._parkPlan.stagePoint);
    if (sp) anchors.push({ x: sp.x, y: sp.y, stage: true });
    anchors.push({
      x: other._cx != null ? other._cx : other.x,
      y: other._cy != null ? other._cy : other.y,
      stage: false
    });
    ...
    for (let a = 0; a < anchors.length; a++) {
      ...
      const latLimit = (reversing || anchor.stage)
        ...
        : PARKING_CONFIG.YIELD_LATERAL;
      if (lat > latLimit) continue;
