...
function updateSignals(dt) {
  if (!signalsEnabled) {
    // Still repaint so lamps go dark when master-off
...
function paintSignalLamps(sig, forceOff) {
  ...
  (sig.heads || []).forEach(head => {
    if (!head.els || !head.els.lamps) return;
    ...
    if (phase) {
      ...
      if (phase.kind === 'allRed' || head.groupIndex !== phase.groupIndex) {
        ...
        head.movements.forEach(m => {
          const c = phase.allow[m];
          if (c) colors.push(c);
        });
        if (colors.length === 0) colors = ['red'];
      }
      // Priority: green > yellow > red for display when mixed (rare)
      if (colors.includes('green')) lit = 'green';
      else if (colors.includes('yellow')) lit = 'yellow';
      else lit = 'red';
    }
    ...
    ['red', 'yellow', 'green'].forEach(color => {
      ...
      if (ag) {
        ...
        ag.querySelectorAll('.signal-arrow-glyph').forEach(p => {
          p.setAttribute('fill', glyphFill);
          p.setAttribute('opacity', glyphOpacity);
        });
...
function updateSignalOpacity(nodeKey) {
  ...
  roots.forEach(r => r.setAttribute('opacity', opacity));
}
