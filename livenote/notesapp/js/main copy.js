...
function loadNote(id) {
  ...
  const otherTabs = document.querySelectorAll('.tab:not([data-id="' + id + '"])');
...
if (Object.keys(notes).length > 0) {
  const firstId = Object.keys(notes).sort((a, b) => {
    const dateA = notes[a].lastType || notes[a].createdAt;
    const dateB = notes[b].lastType || notes[b].createdAt;
