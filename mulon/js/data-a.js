...
async function checkWaitlistAccess(userId, email) {
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      return { approved: true, isExistingUser: true };
    }
...
const MulonData = {
  ...
  // Uses subcollection: mulon_users/{userId}/cards
  // ========================================

  // Add a card to a user (ADMIN ONLY) - adds to subcollection
  async addCardToUser(userId, cardData) {
    ...
    try {
      ...
      // Add the new card to subcollection
      const newCard = {
  ...
  // Remove a specific card from a user (ADMIN ONLY) - removes from subcollection
  async removeCardFromUser(userId, cardNumber) {
    ...
    try {
      ...
      // Find card in subcollection
      const cardsCollectionRef = collection(db, 'mulon_users', userId, 'cards');
      const cardsQuery = query(cardsCollectionRef, where('cardNumber', '==', cardNumber), where('hasCard', '==', true));
      const cardsSnapshot = await getDocs(cardsQuery);
      ...
      // Remove the first matching card (set hasCard to false)
      const cardDoc = cardsSnapshot.docs[0];
      await updateDoc(doc(db, 'mulon_users', userId, 'cards', cardDoc.id), {
        hasCard: false,
  ...
  // Reset a single user's cards (ADMIN ONLY) - clears all cards from subcollection
  async resetUserCards(userId) {
    ...
    try {
      ...
      // Get all cards from subcollection
      const cardsCollectionRef = collection(db, 'mulon_users', userId, 'cards');
      const cardsQuery = query(cardsCollectionRef, where('hasCard', '==', true));
      const cardsSnapshot = await getDocs(cardsQuery);
      const cardsCount = cardsSnapshot.size;
      
      // Mark all cards as not owned
      for (const cardDoc of cardsSnapshot.docs) {
        await updateDoc(doc(db, 'mulon_users', userId, 'cards', cardDoc.id), {
          hasCard: false,
          removedAt: new Date().toISOString(),
          removedBy: 'admin_reset'
        });
      }
  ...
  // Reset ALL users' cards (ADMIN ONLY) - clears all cards from all user subcollections
  async resetAllUsersCards() {
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        ...
        totalCardsDeleted += cardsSnapshot.size;
        
        // Mark all cards as not owned
        for (const cardDoc of cardsSnapshot.docs) {
          await updateDoc(doc(db, 'mulon_users', userDoc.id, 'cards', cardDoc.id), {
            hasCard: false,
            removedAt: new Date().toISOString(),
            removedBy: 'admin_bulk_reset'
          });
        }
  ...
  // Bulk add card to all users (ADMIN ONLY) - adds to subcollections
  ...
  // Bulk remove card from all users (ADMIN ONLY) - removes from subcollections
  async bulkRemoveCardFromAllUsers(cardNumber) {
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        // Find all matching cards in subcollection
        const cardsCollectionRef = collection(db, 'mulon_users', userDoc.id, 'cards');
        const cardsQuery = query(cardsCollectionRef, where('cardNumber', '==', cardNumber), where('hasCard', '==', true));
        const cardsSnapshot = await getDocs(cardsQuery);
        
        if (!cardsSnapshot.empty) {
          // Mark all matching cards as not owned
          for (const cardDoc of cardsSnapshot.docs) {
            await updateDoc(doc(db, 'mulon_users', userDoc.id, 'cards', cardDoc.id), {
              hasCard: false,
              removedAt: new Date().toISOString(),
              removedBy: 'admin_bulk'
            });
            cardsRemoved++;
          }
          updatedCount++;
  ...
  // Get user cards from subcollection (for admin modal refresh)
  async getUserCards(userId) {
    try {
      ...
      const cards = [];
      cardsSnapshot.forEach((cardDoc) => {
        cards.push({ docId: cardDoc.id, ...cardDoc.data() });
      });
      
      console.log(`Loaded ${cards.length} cards for user ${userId} from subcollection`);
      return cards;
    } catch (error) {
  ...
  // Approve custom banner (ADMIN ONLY)
  async approveCustomBanner(userId, bannerUrl, purchaseId) {
    requireAdmin(); // Security check
    ...
    try {
      ...
      await updateDoc(doc(usersRef, userId), {
        ...
        'customizations.custom-profile-banner': true
      });
      
      // Update purchase status
      const purchasesRef = collection(db, 'mulon_purchases');
      await updateDoc(doc(purchasesRef, purchaseId), {
        status: 'completed',
        completedAt: new Date().toISOString()
      });
      
      // Log admin action
      await logAdminAction('approve_custom_banner', {
        targetUserId: userId,
        bannerUrl: bannerUrl,
      ...
      console.error('Error approving custom banner:', error);
      return { success: false, error: error.message };
    }
  },

  // Reject custom banner with refund (ADMIN ONLY)
  async rejectCustomBanner(userId, purchaseId, refundAmount) {
    requireAdmin(); // Security check
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        await updateDoc(doc(usersRef, userId), {
          keys: currentKeys + refundAmount,
          'customizations.custom-profile-banner': false
        });
      }
      ...
      // Log admin action
      await logAdminAction('reject_custom_banner', {
        targetUserId: userId,
        purchaseId: purchaseId,
        refundAmount: refundAmount
      });
      ...
      console.error('Error rejecting custom banner:', error);
      return { success: false, error: error.message };
    }
  },
  ...
  async updatePurchaseStatus(purchaseId, newStatus) {
    ...
    try {
      ...
      await updateDoc(doc(purchasesRef, purchaseId), updateData);
      
      // Log admin action
      await logAdminAction('purchase_status_update', {
        purchaseId: purchaseId,
        newStatus: newStatus
      });
