...
const PARK_STROKE = 'rgba(245, 245, 245, 0.38)';
const PARK_LABEL = 'rgba(245, 245, 245, 0.42)';
const PARK_GHOST_FILL = 'rgba(48, 52, 58, 0.14)';
const PARK_GHOST_STROKE = 'rgba(245, 245, 245, 0.28)';
...
function parkingCurbPoint(seg, along, side) {
  ...
  const px = -uy, py = ux;
  const half = roadBedHalfWidth(seg);
  const t = Math.max(0, Math.min(len, along));
  return {
    x: x1 + ux * t + px * side * half,
    y: y1 + uy * t + py * side * half,
    ux,
    uy,
    nx: px * side,
    ny: py * side,
...
function resolveParkingPick(wx, wy) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const distToCurb = Math.abs(radial - half);
    const distToCenter = Math.hypot(wx - cx, wy - cy);

    if (distToCenter > half + magnet) continue;
    if (radial < half * 0.15 && distToCurb > magnet * 0.35) continue;
    ...
    const candidate = {
      ...
      cy: curb.y,
      dist: distToCurb,
      score: distToCurb + (radial < half ? half - radial : 0) * 0.25,
      roomFwd: 0,
      roomBack: 0,
      dragSign: 1
    };
